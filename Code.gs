...
function setupSheets() {
  ...
  if (sh.getLastRow() === 0) {
    ...
    sh.getRange(2, 1, DEFAULT_STUDENTS.length, 2).setValues(DEFAULT_STUDENTS.map((name, i) => [i + 1, name]));
  ...
  if (sh.getLastRow() === 0) {
    sh.getRange(1, 1, 1, 5).setValues([['Timestamp', 'Nama', 'Tanggal Bayar', 'Jumlah (Rp)', 'Keterangan']]).setFontWeight('bold');
  }

  sh = ss.getSheetByName(SHEET_PENGELUARAN);
  if (!sh) sh = ss.insertSheet(SHEET_PENGELUARAN);
  if (sh.getLastRow() === 0) {
    sh.getRange(1, 1, 1, 4).setValues([['Timestamp', 'Tanggal', 'Deskripsi', 'Jumlah (Rp)']]).setFontWeight('bold');
  }
  ...
  if (sh.getLastRow() === 0) {
    sh.getRange(1, 1, 5, 2).setValues([
      ['Key', 'Value'], ['NamaKelas', 'Kas Kelas 12.5'], ['TanggalMulai', formatISO(new Date())],
      ['IuranMingguan', 5000], ['SaldoAwal', 0]
    ]);
...
function getStudentStatus(nama) {
  ...
  const data = getSS().getSheetByName(SHEET_PEMBAYARAN).getDataRange().getValues();
  let totalPaid = 0;
  const history = [];
  for (let i = 1; i < data.length; i++) {
    if (data[i][1] === nama) {
      ...
      totalPaid += jumlah;
      history.push({ tanggal: formatTanggalID(data[i][2]), tanggalRaw: new Date(data[i][2]).getTime(), jumlah: jumlah, keterangan: data[i][4] || '' });
    }
  }
  history.sort((a, b) => b.tanggalRaw - a.tanggalRaw);
  const kurang = totalDue - totalPaid;
  return { nama, weeklyFee, weeksElapsed, totalDue, totalPaid, kurang, lunas: kurang <= 0, history };
}
...
function getStudentStatuses() {
  ...
  return students.map(nama => {
    ...
    return { nama, totalPaid, totalDue: totalDuePerStudent, kurang, lunas: kurang <= 0 };
  });
}
...
function getAdminDashboard(pin) {
  ...
  for (let i = 1; i < expRows.length; i++) {
    ...
    recentExpenses.push({ tanggal: formatTanggalID(expRows[i][1]), deskripsi: expRows[i][2], jumlah });
  }
  ...
  for (let i = 1; i < payRows.length; i++) {
    recentPayments.push({ nama: payRows[i][1], tanggal: formatTanggalID(payRows[i][2]), jumlah: Number(payRows[i][3]) || 0, keterangan: payRows[i][4] || '' });
  }
  recentPayments.reverse();

  return {
    namaKelas: getSetting('NamaKelas'), weeklyFee, weeksElapsed,
    tanggalMulai: formatTanggalID(getSetting('TanggalMulai')),
    tanggalMulaiISO: formatISO(new Date(getSetting('TanggalMulai'))), saldoAwal,
    saldoSaatIni, totalTerkumpul, totalPengeluaran,
    jumlahLunas: studentStatus.filter(s => s.lunas).length,
    jumlahBelumLunas: studentStatus.filter(s => !s.lunas).length,
    students: studentStatus, recentPayments: recentPayments.slice(0, 15), recentExpenses: recentExpenses.slice(0, 15)
  };
}
...
function addExpense(pin, tanggal, deskripsi, jumlah) {
  ...
  sh.appendRow([new Date(), tanggal ? new Date(tanggal + 'T00:00:00') : new Date(), deskripsi, amount]);
  invalidateTransparencyCache();
...
function updateSettings(pin, tanggalMulai, iuranMingguan, saldoAwal) {
  ...
  if (saldoAwal !== null && saldoAwal !== undefined && saldoAwal !== '') setSetting('SaldoAwal', Number(saldoAwal));
  invalidateTransparencyCache();
...
function addStudent(pin, nama) {
  ...
  const sh = getSS().getSheetByName(SHEET_SISWA);
  const data = sh.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) if (String(data[i][1]).trim().toLowerCase() === nama.toLowerCase()) throw new Error('Nama siswa sudah ada.');
...
function syncSheet1() {
  ...
  if (!studentSh || !paymentSh) throw new Error('Sheet Siswa/Pembayaran belum tersedia. Jalankan setupSheets() terlebih dahulu.');

  const students = studentSh.getDataRange().getValues().slice(1)
    .filter(r => String(r[1] || '').trim() !== '').map(r => String(r[1]).trim());
  ...
  if (oldRows > 0) sh.getRange(firstDataRow, 1, oldRows, dataCols).clearContent();
  ...
  // Jangan memanggil setFrozenColumns(2) setiap transaksi.
  // Sheet1 dapat memiliki merged cells pada header A:B, sehingga Google Sheets
  // melempar 405/exception ketika diminta membekukan hanya sebagian merged range.
  // Freeze columns adalah pengaturan layout, bukan bagian dari sinkronisasi data.
  // Biarkan konfigurasi freeze yang sudah ada di spreadsheet tetap apa adanya.
  sh.setFrozenRows(4);
  SpreadsheetApp.flush();
...
function invalidateTransparencyCache() {
  try { CacheService.getScriptCache().remove(TRANSPARENCY_CACHE_KEY); }
  catch (err) { console.error('invalidateTransparencyCache:', err); }
}
...
function getTransparencyData() {
  ...
  for (let i = 1; i < payRows.length; i++) {
    ...
    transactions.push({ tanggalRaw: new Date(payRows[i][2]), jenis: 'masuk', deskripsi: 'Iuran kas — ' + payRows[i][1], jumlah: Number(payRows[i][3]) || 0 });
  }
  for (let i = 1; i < expRows.length; i++) {
    if (!expRows[i][1]) continue;
    transactions.push({ tanggalRaw: new Date(expRows[i][1]), jenis: 'keluar', deskripsi: expRows[i][2], jumlah: Number(expRows[i][3]) || 0 });
  }
  ...
  const totalTransactions = transactions.map(t => {
    ...
    return { tanggal: formatTanggalID(t.tanggalRaw), tanggalRaw: t.tanggalRaw.getTime(), jenis: t.jenis, deskripsi: t.deskripsi, jumlah: t.jumlah, saldo };
  }).reverse();
  ...
  const result = {
    namaKelas: settings['NamaKelas'], saldoAwal, saldoAkhir: saldo,
  ...
  try { cache.put(TRANSPARENCY_CACHE_KEY, JSON.stringify(result), TRANSPARENCY_CACHE_SECONDS); }
  catch (err) { console.error('cache.put transparansi:', err); }
  return result;
}
...
function getTransparencyDashboard() {
  ...
  const chronological = data.transactions.slice().reverse();
  const daily = {};
  const order = [];
  ...
  const recentDays = order.slice(-14);
  return {
    saldo: data.saldoAkhir, totalMasuk: data.totalPemasukan, totalKeluar: data.totalPengeluaran,
    mingguBerjalan: weeksElapsed, lunas, belumLunas,
    labels: recentDays, balance: recentDays.map(k => daily[k].saldo),
    income: recentDays.map(k => daily[k].in), expense: recentDays.map(k => daily[k].out),
